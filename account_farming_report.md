# Account Farming Report

- **JARJARBINKS**: G9 → G13, R1 → R5

## Łączne braki

| ID | Przedmiot | Wymagane | Posiadane | Brakuje |
|---|---|---:|---:|---:|
| `144` | Mk 6 Merr-Sonn Thermal Detonator | 3 | 0 | **3** |
| `129` | Mk 8 BioTech Implant | 2 | 0 | **2** |
| `168` | Mk 12 ArmaTek Fusion Furnace | 2 | 0 | **2** |
| `174` | Mk 7 Kyrotech Power Converter | 2 | 0 | **2** |
| `116` | Mk 4 Chedak Comlink | 1 | 0 | **1** |
| `125` | Mk 7 Nubian Security Scanner | 1 | 0 | **1** |
| `134` | Mk 9 BAW Armor Mod | 1 | 0 | **1** |
| `142` | Mk 6 A/KT Stun Gun | 1 | 0 | **1** |
| `146` | Mk 10 Neuro-Saav Electrobinoculars | 1 | 0 | **1** |
| `158` | Mk 12 ArmaTek Multi-tool | 1 | 0 | **1** |
| `159` | Mk 12 ArmaTek Armor Plating | 1 | 0 | **1** |
| `160` | Mk 12 ArmaTek Bayonet | 1 | 0 | **1** |
| `G12Finisher_JARJARBINKS_C` | G12Finisher_JARJARBINKS_C | 1 | 0 | **1** |

## Szczegóły

### JARJARBINKS

Gear: **G9 → G13** Relic: **R1 → R5**

#### Bezpośredni wymagany gear

- `116` × 1 — Mk 4 Chedak Comlink
- `125` × 1 — Mk 7 Nubian Security Scanner
- `129` × 2 — Mk 8 BioTech Implant
- `134` × 1 — Mk 9 BAW Armor Mod
- `142` × 1 — Mk 6 A/KT Stun Gun
- `144` × 3 — Mk 6 Merr-Sonn Thermal Detonator
- `146` × 1 — Mk 10 Neuro-Saav Electrobinoculars
- `158` × 1 — Mk 12 ArmaTek Multi-tool
- `159` × 1 — Mk 12 ArmaTek Armor Plating
- `160` × 1 — Mk 12 ArmaTek Bayonet
- `168` × 2 — Mk 12 ArmaTek Fusion Furnace
- `174` × 2 — Mk 7 Kyrotech Power Converter
- `G12Finisher_JARJARBINKS_C` × 1 — G12Finisher_JARJARBINKS_C

#### Materiały relic


#### Braki

| ID | Przedmiot | Brakuje |
|---|---|---:|
| `144` | Mk 6 Merr-Sonn Thermal Detonator | **3** |
| `129` | Mk 8 BioTech Implant | **2** |
| `168` | Mk 12 ArmaTek Fusion Furnace | **2** |
| `174` | Mk 7 Kyrotech Power Converter | **2** |
| `116` | Mk 4 Chedak Comlink | **1** |
| `125` | Mk 7 Nubian Security Scanner | **1** |
| `134` | Mk 9 BAW Armor Mod | **1** |
| `142` | Mk 6 A/KT Stun Gun | **1** |
| `146` | Mk 10 Neuro-Saav Electrobinoculars | **1** |
| `158` | Mk 12 ArmaTek Multi-tool | **1** |
| `159` | Mk 12 ArmaTek Armor Plating | **1** |
| `160` | Mk 12 ArmaTek Bayonet | **1** |
| `G12Finisher_JARJARBINKS_C` | G12Finisher_JARJARBINKS_C | **1** |

## Informacje techniczne

- Inventory pochodzi z `c3po.json`.
- Aktualny gear/relic pochodzi z `roster.csv`.
- Wymagany gear pochodzi z `unitTier[].equipmentSet`.
- Receptury gearu są rozwijane rekurencyjnie.
- Materiały reliców pochodzą z `relic_promotion_recipe_01...10`.
- `9999` jest traktowane jako placeholder G13 i nie jest liczone jako gear.